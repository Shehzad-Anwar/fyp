...
const OrderDetails = ({ order, Check, ...props }) => {
  if (Check == false) {
    ...
    useEffect(() => {
      ...
      // console.log(Status.status, Step);
      const d = new Date(order.createdAt);
      setDate(d);
    }, [Status]);

    const onUpdate = async (e, orderID) => {
      setStatus({ status: e.target.value });
      let response = await fetch(
        `${process.env.NEXT_PUBLIC_HOST}/api/Order/update?orderID=${orderID}`,
        {
          method: "POST", // or 'PUT'
          headers: {
                    ...
                    <main className="mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
                      ...
                      <div className="mt-2 border-b border-gray-200 pb-5 text-sm sm:flex sm:justify-between">
                        <dl className="flex ">
                          ...
                          <dd className="font-medium text-gray-900 ml-32">
                            <select
                              id="orderStatus"
                              name="orderStatus"
                              value={Status.status}
                              onChange={(e) => onUpdate(e, order.orderID)}
                                ...
                                <td className="py-6">
                                  <div className="flex items-center">
                                    <img
                                      ...
                                      className="mr-6 h-16 w-16 rounded object-center object-contain"
                                    />
                                    <div>
                                ...
                                <div className="mt-6">
                                  ...
                                  <div className="mt-6 hidden grid-cols-5 text-gray-600 sm:grid">
                                    ...
                                    <div
                                      className={
                                        Step >= 2
                                          ? "text-indigo-600 font-bold text-center cursor-pointer"
                                          : Step == 4
                                          ? "text-green-500 font-bold text-center cursor-pointer"
  ...
  if (Check) {
    ...
    if (context.query.id) {
      order = await Order.findOne(
        ...
        { _id: 0, sellQty: 0, adminId: 0, createdAt: 0, updatedAt: 0, userId: 0 }
      );
